...
for team in tqdm(tba.district_teams('2023chs')):
    nickname = tba.team(team.key, simple=True).nickname
    ...
    plt.title("Match Win Record for FRC " + str(team.key[3:]) + ": " +
              nickname, fontsize='x-large')

    plt.xlim([-0.05, x[-1] + 0.05])
    plt.ylim([ymin - 1, ymax + 1])
    plt.box(False)
    plt.grid(axis='y', linestyle='dotted')
    plt.gca().get_xaxis().set_visible(False)
    plt.ylabel("Net Match Wins", fontsize='large')
    plt.savefig(str(team.key[3:]) + ' Wins Record.png')
    plt.close()
